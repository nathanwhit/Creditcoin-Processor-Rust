...
fn close_deal_order_success() {
    ...
    let mut invest_transfer_id = AddressId::with_prefix_key(
        TRANSFER.clone(),
        &string!(
    ...
    let mut invest_transfer = crate::protos::Transfer {
        ...
        order: register_transfer_invest.order_id.clone(),
        amount: deal_order.amount.clone(),
        tx: register_transfer_invest.blockchain_tx_id.clone(),
        sighash: investor.clone().to_string(),
    ...
    let mut repayment_transfer_id = AddressId::with_prefix_key(
        TRANSFER.clone(),
        &string!(
            &investor_address.blockchain,
    ...
    let mut repayment_transfer = crate::protos::Transfer {
        ...
        order: register_transfer_repayment.order_id.clone(),
        amount: amount.clone().into(),
        tx: register_transfer_repayment.blockchain_tx_id.clone(),
        sighash: fundraiser.clone().to_string(),
